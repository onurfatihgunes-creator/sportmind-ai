 ...
 * (BSD-sourced Süper Lig rows bsd-215984/985 were stuck 'scheduled' with real scores).
 * A provider-official result that is not literally FINISHED is also honored: Ligue 1 542704
 * is AWARDED with winner DRAW and 0-0 (verified against football-data.org, 2026-09-24;
 * BSD calls the same match canceled) — the provider's own result, so it stays finished,
 * while team_form (recorded only for a provider-FINISHED match) rightly excludes it.
 * NOTE: `score.winner` is NOT a placeholder discriminator — football-data.org also sends
 * winner DRAW with 0-0 for TIMED future fixtures (La Liga 564685, verified), which is why
 * the kickoff-age guard, not the winner, is what protects those.
 */
...
export function resolveFootballMatchStatus(
  ...
  // CANCELLED folds into 'postponed' exactly as the BSD/RapidAPI paths already collapse their
  // own cancelled state (the matches.status check constraint has no room for a 4th value).
 ...
 * cancelled fixture (La Liga 564685; balldontlie does the same for NBA, see
